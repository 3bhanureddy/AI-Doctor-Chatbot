...
async def upload_and_query(image: UploadFile = File(...), query: str = Form(...)):
    try:
        ...
        if scout_response.status_code == 200:
            result = scout_response.json()
            answer = result["choices"][0]["message"]["content"]
            logger.info(f"Processed response from Scout API: {answer[:100]}...")
            return JSONResponse(status_code=200, content={"llama": answer})
        else:
            logger.error(f"Error from Scout API: {scout_response.status_code} - {scout_response.text}")
            raise HTTPException(status_code=scout_response.status_code, detail=scout_response.text)
